...
for index, row in merged_data.iterrows():
    ...
    sign_class = sign_class_dict.get(row['Sign_class'], 'Desconhecido')
    ws.cell(row=index+2, column=1, value=sign_class)
    ws.cell(row=index+2, column=2, value=row['ID'])  # Coluna 'ID'
    ws.cell(row=index + 2, column=5, value=row['HoraMinSeg'])  # Coluna 'Horario'

    # Centralizar todos os valores
    for col in range(1, 8):  # Colunas 'Classe', 'ID', 'Imagem', 'Horario', 'Pista (Simples/Dupla)', 'Sentido', 'Data da medição'
...
for index, row in merged_data.iterrows():
    img_path_inteira = fr"Resultado_de_dados\imagens_inteiras\foto_inteira_do_id_{int(row['ID'])}.jpg"  # Nome do arquivo de imagem
    img = Image(img_path_inteira)

    # Definir o tamanho da imagem para um tamanho fixo
    img.width = fixed_image_inteirias_width
    img.height = fixed_image_inteiras_height

    # Inserir a imagem na célula E especifica
    cell_e = ws.cell(row=index+2, column=4)  # Coluna 'Imagem'
    ws.add_image(img, cell_e.coordinate)  # Insere a imagem na célula E correspondente
